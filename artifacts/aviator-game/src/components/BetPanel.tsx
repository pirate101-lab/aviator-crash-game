...
export default function BetPanel({
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-lg border border-white/8 p-3">
      {/* Tab switcher */}
      ...
      <div className="flex items-stretch gap-3">
        ...
        <div className="flex-1 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={decrease}
              className="w-9 h-9 rounded-full border border-white/20 bg-[#0d0d0d] flex items-center justify-center text-white/70 text-lg font-bold hover:border-[#00E676]/40 hover:text-white active:scale-95 transition-all flex-shrink-0"
            >
              −
            </button>
            <div className="flex-1 text-center font-mono text-base font-bold text-white">
          ...
          <div className="grid grid-cols-4 gap-1">
            {QUICK_AMOUNTS.map((v) => (
              <button
                ...
                type="button"
                onClick={() => setAmount(v)}
                className="rounded bg-[#0d0d0d] border border-white/10 py-1 text-[10px] font-mono text-white/50 hover:border-[#00E676]/30 hover:text-white/80 transition-all"
              >
                {v >= 1000 ? `${v / 1000}k` : v}
              </button>
            ))}
        ...
        <button
          type="button"
          className="flex-shrink-0 flex flex-col items-center justify-center rounded-xl bg-[#00E676] px-5 py-3 hover:brightness-110 hover:scale-[1.02] active:scale-[0.97] transition-all min-w-[90px]"
        >
          <span className="text-lg font-black text-black leading-tight">BET</span>
          <span className="text-[11px] font-semibold text-black/75 font-mono leading-tight">
